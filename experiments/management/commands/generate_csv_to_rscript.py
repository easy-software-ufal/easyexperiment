    ...
    def handle(self, *args, **options):
        ...
        with open('C:/Users/nando/OneDrive/Documentos/datasetatoms_v2.csv', 'wb') as csvfile:
            ...
            for index, latin_square in enumerate(latin_squares, start=1):
                ...
                # write lines to csv
                filewriter.writerow(
                    [index, latin_square.id, 1, latin_square.row1.participant.name.encode('utf-8'), "ST1",
                     tasks_row1_cell1, row1_cell1_type, trials_row1_cell_1, duration_row1_cell_1, duration_row1_cell_1])
                filewriter.writerow(
                    [index, latin_square.id, 1, latin_square.row1.participant.name.encode('utf-8'), "ST2",
                     tasks_row1_cell2, row1_cell2_type, trials_row1_cell_2, duration_row1_cell_2, duration_row1_cell_2])
                filewriter.writerow(
                    [index, latin_square.id, 2, latin_square.row2.participant.name.encode('utf-8'), "ST1",
                     tasks_row2_cell1, row2_cell1_type, trials_row2_cell_1, duration_row2_cell_1, duration_row2_cell_1])
                filewriter.writerow(
                    [index, latin_square.id, 2, latin_square.row2.participant.name.encode('utf-8'), "ST2",
                     tasks_row2_cell2, row2_cell2_type, trials_row2_cell_2, duration_row2_cell_2, duration_row2_cell_2])
